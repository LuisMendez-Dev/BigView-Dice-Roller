  ...
  return (
    <>
      {selectedDices.length === 0 && (
        <p className='my-10 w-full text-center font-extralight text-gray-700'>
      ...
      {selectedDices.length > 0 && (
        <div
          className={`grid grid-cols-3 border-2 ${selectedDices.length > 0 ? 'border-neutral-300' : 'border-transparent'} h-auto w-full rounded-md p-2`}
        >
          {selectedDices.length > 0 &&
            selectedDices.map((dice, index) => (
              ...
              >
                ...
                <CardBody className='flex items-center justify-center'>
                  {loadingState ? (
                    <p className='animate-bounce rounded-full text-center text-6xl font-semibold text-black'>
                      ...
                    </p>
                  ) : (
                    <img
                ...
                {rollResult && rollResult.length > 0 && (
                  <CardFooter className='flex items-center justify-center text-center font-semibold lg:h-10 2xl:mb-2'>
                    <p className='font-base rounded-full text-center text-xl text-black 2xl:text-2xl'>
                      {rollResult &&
                        rollResult[index] &&
